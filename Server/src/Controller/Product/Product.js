...
const requiredFieldsForCreate = (body) => {
  ...
  // Img1/2/3 are required files (checked separately)
  if (!norm(body.Cline1)) missing.push("Cline1");
  if (!norm(body.Cline2)) missing.push("Cline2");
  if (!norm(body.Cline3)) missing.push("Cline3");
  if (!norm(body.Cline4)) missing.push("Cline4");
  if (!norm(body.Cline5)) missing.push("Cline5");
  if (!norm(body.Cline6)) missing.push("Cline6");

  // Technical specs
  for (let i = 1; i <= 10; i++) {
    if (!norm(body[`Tspec${i}`])) missing.push(`Tspec${i}`);
  }
  // Applications
  for (let i = 1; i <= 6; i++) {
    if (!norm(body[`App${i}`])) missing.push(`App${i}`);
  }
...
export const createProduct = async (req, res) => {
  ...
  try {
    ...
    // Upload images to Cloudinary (in transaction but commit later)
...
export const updateProduct = async (req, res) => {
  ...
  try {
    ...
    // update Cline/Tspec/App fields if present
    ...
    // Catagory/Subcat validation if present
    if (typeof req.body.CatagoryId !== "undefined") {
      const catId = norm(req.body.CatagoryId);
      if (!catId) {
        if (session.inTransaction()) await session.abortTransaction();
